...
//Algorithm provided by Rosetta Code

typedef unsigned int uint;

using namespace std;

const uint TAPE_MAX_LEN = 50000;

struct action{
    char write, direction;
};

class tape{

    public:
        tape( uint startPos = TAPE_MAX_LEN >> 1 ) : MAX_LEN( TAPE_MAX_LEN ){
            _sp = startPos;
            reset();
        }
        ...
        void input(string a){
            if( a == "" ) return;
            for(uint s = 0;s < a.length(); s++) _t[headPos + s] = a[s];
        }

        void clear(char c){
            _t.clear();
            blk = c;
            _t.resize( MAX_LEN, blk );
        }
        ...
        void print( int c = 10 ){
        int ml = static_cast<int>(MAX_LEN),
        st = static_cast<int>(headPos) - c,
        ed = static_cast<int>( headPos ) + c + 1,
        tx;
        for(int x = st; x < ed; x++){
        tx = x;
        if( tx < 0 ) tx += ml;
    ...
    private:
        void move(char d){
            if( d == 'N' ) return;
            headPos += d == 'R' ? 1 : -1;
            if( headPos >= MAX_LEN ) headPos = d == 'R' ? 0 : MAX_LEN - 1;
        }
        void write(char a){
            if( a != 'N' ){
            if( a == 'B' ) _t[headPos] = blk;
            else _t[headPos] = a;
            }
        }
...
public:
    ...
    utm(){
        files[0] = "incrementer.utm";
        files[1] = "busy_beaver.utm";
        files[2] = "sort.utm";
    }
    ...
    void start(){
        while(true){
            ...
            int t = showMenu();
            if(t == 0) return;
            if(!at.loadTable(files[t - 1])) return;
            startMachine();
        }
    }
private:
    void simulate(){
        char r;
        action a;
        while( true ) { tp.print(); r = tp.read();
        if( !( at.action( r, a ) ) ) break;
        tp.action( &a ); }
	...
	while(t < 0 || t > 3){
	    system( "cls" );
        cout << "1. Incrementer\n2. Busy beaver\n3. Sort\n\n0. Quit";
	    cout << endl << endl << "Choose an action ";
        cin >> t;
	}
	return t;
    }
    ...
    void startMachine(){
        system( "cls" );
        tp.clear(at.getBlank());
        tp.input(at.getInput());
        simulate();
    }
